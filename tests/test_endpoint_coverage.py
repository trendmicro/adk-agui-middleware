...
class TestEndpointCoverage(BaseTestCase):
    ...
    async def test_endpoint_function_success(self):
        ...
        endpoint_func = None
        for route in self.app.routes:
            if hasattr(route, 'endpoint') and route.endpoint.__name__ == 'agui_endpoint':
        ...
        with patch('adk_agui_middleware.endpoint.exception_http_handler') as mock_handler:
    ...
    async def test_endpoint_function_exception_handling(self):
        ...
        with patch('adk_agui_middleware.endpoint.exception_http_handler') as mock_handler:
            mock_handler.return_value.__aenter__ = AsyncMock(side_effect=HTTPException(500, "Internal error"))
    ...
    async def test_endpoint_encoder_initialization(self):
        ...
        with patch('adk_agui_middleware.endpoint.exception_http_handler') as mock_handler:
            with patch('adk_agui_middleware.endpoint.EventEncoder', return_value=mock_encoder) as mock_encoder_class:
                mock_handler.return_value.__aenter__ = AsyncMock()
                mock_handler.return_value.__aexit__ = AsyncMock()
    ...
    async def test_service_integration(self):
        ...
        with patch('adk_agui_middleware.endpoint.exception_http_handler') as mock_handler:
            with patch('adk_agui_middleware.endpoint.EventEncoder', return_value=mock_encoder):
                ...
                self.mock_service.get_runner.assert_called_once_with(agui_content, request)
                self.mock_service.event_generator.assert_called_once_with(mock_runner, mock_encoder)
