...
class TestErrorEventCoverage(BaseTestCase):
    ...
    def test_agui_encoder_error_encoding_error(self):
        ...
        with patch('adk_agui_middleware.event.error_event.record_error_log') as mock_log:
    ...
    def test_agui_error_event_no_tool_results(self):
        ...
        with patch('adk_agui_middleware.event.error_event.record_error_log') as mock_log:
            ...
            expected_log_message = f"Tool result submission without tool results for thread {thread_id}"
    ...
    def test_agui_error_event_tool_result_processing_error(self):
        ...
        with patch('adk_agui_middleware.event.error_event.record_error_log') as mock_log:
            ...
            mock_log.assert_called_once_with("Error handling tool results.", test_exception)
    ...
    def test_encoder_error_with_complex_exception(self):
        ...
        try:
            ...
            with patch('adk_agui_middleware.event.error_event.record_error_log'):
                result = AGUIEncoderError.encoding_error(mock_encoder, complex_exception)
    ...
    def test_agent_error_with_custom_exception(self):
        ...
        custom_exception = CustomAgentException("AGENT_TIMEOUT", "Agent took too long to respond")
    ...
    def test_tool_result_processing_with_unicode_error(self):
        ...
        with patch('adk_agui_middleware.event.error_event.record_error_log') as mock_log:
            ...
            mock_log.assert_called_once_with("Error handling tool results.", unicode_exception)
    ...
    def test_error_message_formatting(self):
        ...
        with patch('adk_agui_middleware.event.error_event.record_error_log'):
            ...
            processing_result = AGUIErrorEvent.tool_result_processing_error(test_exception)
            assert f"Failed to process tool results: {repr(test_exception)}" == processing_result.message
    ...
    def test_event_type_consistency(self):
        ...
        with patch('adk_agui_middleware.event.error_event.record_error_log'):
            ...
            processing_event = AGUIErrorEvent.tool_result_processing_error(test_exception)
    ...
    def test_error_codes_uniqueness(self):
        ...
        with patch('adk_agui_middleware.event.error_event.record_error_log'):
            ...
            expected_codes = {"EXECUTION_ERROR", "NO_TOOL_RESULTS", "TOOL_RESULT_PROCESSING_ERROR"}
