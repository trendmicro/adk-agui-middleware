...
class TestShutdownCoverage(BaseTestCase):
    ...
    async def test_graceful_shutdown_with_error(self):
        ...
        with patch.object(handler, 'close', new=AsyncMock(side_effect=Exception("Shutdown error"))):
            with patch('adk_agui_middleware.tools.shutdown.record_error_log') as mock_error_log:
                await handler._graceful_shutdown()
