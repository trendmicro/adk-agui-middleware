...
from ag_ui.core import AssistantMessage, RunAgentInput, ToolCall, ToolMessage, UserMessage, FunctionCall
...
class TestUserMessageHandler(unittest.TestCase):
    ...
    async def test_get_message_tool_result_priority(self):
        ...
        messages = [
            ...
            AssistantMessage(id="2", role="assistant", content="", tool_calls=[tool_call]),
            ToolMessage(id="3", role="tool", tool_call_id="call_1", content='{"result": "data"}')
        ]
        handler = self.create_handler(messages=messages)
    ...
    def test_multiple_tool_calls_mapping(self):
        ...
        tool_call1 = ToolCall(
            id="call_1",
            function=FunctionCall(name="function_1", arguments="{}")
        )
        tool_call2 = ToolCall(
            id="call_2", 
            function=FunctionCall(name="function_2", arguments="{}")
        )
