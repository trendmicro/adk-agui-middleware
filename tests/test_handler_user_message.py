...
class TestUserMessageHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_agui_content = MockRunAgentInput(
            thread_id="test_thread_123", messages=[]
        )
    ...
    def test_init_without_initial_state(self):
        ...
        handler = UserMessageHandler(
            agui_content=self.mock_agui_content, request=self.mock_request
        )
    ...
    async def test_extract_tool_results_with_tool_message(self, mock_log):
        ...
        # Create tool call
        tool_call = ToolCall(
            id="tool_call_123", type="function", function=Mock(name="test_function")
        )
    ...
    async def test_extract_tool_results_multiple_tool_messages(self):
        ...
        tool_msg1 = ToolMessage(
            id="tool_1", role="tool", content="First result", tool_call_id="call_1"
        )

        tool_msg2 = ToolMessage(
            id="tool_2", role="tool", content="Latest result", tool_call_id="call_2"
        )
    ...
    async def test_process_tool_results_with_valid_json(self):
        ...
        self.handler.extract_tool_results = AsyncMock(
            return_value=[{"tool_name": "test_tool", "message": tool_msg}]
        )
    ...
    async def test_process_tool_results_with_invalid_json(self):
        ...
        tool_msg = ToolMessage(
            id="tool_1", role="tool", content="invalid json", tool_call_id="call_123"
        )
    ...
    async def test_get_message_user_message(self):
        ...
        with patch.object(
            self.handler, "process_tool_results", AsyncMock(return_value=None)
        ):
            with patch.object(
                self.handler,
                "get_latest_message",
    ...
    async def test_get_message_no_content(self):
        ...
        with patch.object(
            ...
            with patch.object(
                self.handler, "get_latest_message", AsyncMock(return_value=None)
            ):
                result = await self.handler.get_message()
    ...
    async def test_integration_full_conversation_flow(self):
        ...
        tool_call = ToolCall(
            id="integrate_call", type="function", function=Mock(name="integration_tool")
        )
