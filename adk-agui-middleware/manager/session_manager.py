...
from loggers.record_log import record_warning_log, record_log, record_error_log
...
class SessionManager:
    ...
    async def get_or_create_session(
        ...
        if self.tracked_sessions.check_user_session_limit(session_parameter.user_id, session_parameter.session_id):
            record_warning_log(
    ...
    async def update_session_state(
        ...
        session = await self.session_service.get_session(
            session_id=session_parameter.session_id, app_name=session_parameter.app_name,
            user_id=session_parameter.user_id
        )
        if not (session and state_updates):
            record_warning_log(f"Session not found: {session_parameter.app_name}:{session_parameter.session_id}")
            return False
        event = Event(
...
class SessionHandler:
    ...
    def __init__(self, session_manger: SessionManager, session_parameter: SessionParameter):
    ...
    async def add_pending_tool_call(self, tool_call_id: str):
        ...
        try:
            ...
            record_error_log(
                f"Failed to add pending tool call {tool_call_id} to session {self.session_parameter.session_id}.", e
            )

    async def get_pending_tool_calls(self) -> Any | None:
        try:
            session_history = self.session_manger.user_sessions.get(self.session_parameter.user_id)
            ...
            record_error_log(f"Failed to check pending tool calls for session {self.session_parameter.session_id}.", e)
        return None
